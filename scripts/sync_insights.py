...
def main():
    """Upsert records by slug — preserve user-curated fields.

    Strategy:
      1. Build records from scraped JSON files.
      2. Pull existing records from Bitable, index by slug.
      3. For records that exist: update content fields ONLY (preserve is_pinned,
         is_homepage_hero, is_featured, tags, tags_form — these are user-edited).
      4. For new records: create.
      5. Records in Bitable not in current scrape: leave alone (archive).
    """
    # Fields we never overwrite on an existing record.
    PRESERVE = {"is_pinned", "is_homepage_hero", "is_featured", "tags", "tags_form"}

    # 1. Build incoming records.
    dy = make_douyin_records()
    xhs = make_xhs_records()
    wx = make_wechat_records()
    new_recs = dy + xhs + wx
    print(f"Built: douyin={len(dy)} xhs={len(xhs)} wechat={len(wx)} total={len(new_recs)}")
    if not new_recs:
        print("Nothing to sync.")
        return

    # 2. Index existing records by slug.
    print("Loading existing records …")
    existing = list_all(TBL_INSIGHTS)
    by_slug = {}
    for it in existing:
        s = it["fields"].get("slug")
        ...
        if s:
            by_slug[str(s)] = it["record_id"]
    print(f"  existing in Bitable: {len(by_slug)}")

    # 3. Split into create / update.
    to_create = []
    to_update = []
    for rec in new_recs:
        slug = rec.get("slug")
        if slug and slug in by_slug:
            update_fields = {k: v for k, v in rec.items() if k not in PRESERVE}
            to_update.append({"record_id": by_slug[slug], "fields": update_fields})
        else:
            to_create.append({"fields": rec})
    print(f"  plan: create={len(to_create)} update={len(to_update)} "
          f"(preserve fields: {sorted(PRESERVE)})")
    ...
    for i in range(0, len(to_create), 100):
        ...
        if r.get("code") != 0:
            print(f"  ! create batch {i // 100 + 1}: {r.get('msg')}")
            print(json.dumps(batch[0], ensure_ascii=False)[:600])
            return
        print(f"  ✓ created batch {i // 100 + 1}: {len(batch)}")

    # 5. Execute updates.
    for i in range(0, len(to_update), 100):
        batch = to_update[i:i + 100]
        r = post(
            f"/open-apis/bitable/v1/apps/{APP}/tables/{TBL_INSIGHTS}/records/batch_update",
            {"records": batch},
        )
        if r.get("code") != 0:
            print(f"  ! update batch {i // 100 + 1}: {r.get('msg')}")
            print(json.dumps(batch[0], ensure_ascii=False)[:600])
            return
        print(f"  ✓ updated batch {i // 100 + 1}: {len(batch)}")

    # 6. Report orphans (records in Bitable not in current scrape).
    incoming_slugs = {r.get("slug") for r in new_recs}
    orphans = [s for s in by_slug if s not in incoming_slugs]
    if orphans:
        print(f"\n  ⓘ {len(orphans)} records in Bitable not in current scrape (kept as archive)")

    print(f"\n✅ Upsert complete: {len(to_create)} created, {len(to_update)} updated")
